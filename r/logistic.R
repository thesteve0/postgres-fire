...
typeof(logmodel_solar)
typeof(summary(logmodel_solar))

# I am going to return the binary representation
## we get back raw
### Won't work according to Joe C - just return the model
...
## We get back a list
### According to Joe C. this will not work
...
df_test <- dbGetQuery(con,statement = paste("select hasfire, date_time, cs_precip, cs_air_max_temp,",
                                                  "cs_air_min_temp, cs_soil_max_temp, cs_soil_min_temp, cs_solar, cs_eto, cs_rh_max, cs_rh_min, hasfire",
                                                  "from final.test order by date_time limit 1"))
df_test <- dbGetQuery(con,statement = paste("select * from final.test order by date_time limit 1"))

prob <- predict(logmodel_solar, df_test, type = 'response')
prob
toString(df_test)
prob
